...
def Simulation(sample_pts, p0, v0, Dr, pinning_percent, L, h, init_steps, n_steps,msd_step, set_i, save_coordinate=False):
    kp = 1
    A0 = 1
    l_dash=0.60
    initial_steps_pin=1000
    ...
    for step in range(initial_steps_pin+1):
        pts = peridodic_points(duplicate_pts, L)
        Tri = Delaunay(pts)
        tri = Tri.simplices
        nei = Tri.neighbors
    ...
    for step in range(n_steps+msd_step+1):
        pts = peridodic_points(duplicate_pts, L)
        Tri = Delaunay(pts)
        tri = Tri.simplices
        nei = Tri.neighbors
...
msd_step=100000
set_i=1
Simulation(pts, p0, v0, Dr,  pinning_percent, L, dt,initial_steps, n_steps,msd_step,  set_i, save_coordinate=True)
